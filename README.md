...
详细的版本更新记录见 [CHANGELOG.md](./CHANGELOG.md)。这里简单说说最近在折腾什么，主要是设计上返工的几轮。

**v7.3 — 修文字溢出**

写着用着发现有文字超出了边框：图书管理那个表格整个宽出去一截，弹窗右上角的关闭按钮贴着边。这种问题肉眼一看就有，之前居然没查出来，说明审查方式不对——光盯着 CSS 数值看没用，得一个元素一个元素量。所以写了个小脚本，把三个角色的页面和能点开的弹窗都扫了一遍，查出来几处：表格列宽压了压，关闭按钮离边远了点，读者首页推荐标签太长会戳出卡片、改成换行。改完又扫了一遍，没再查出新的。

**v7.2 — 去渐变**

觉得渐变色一股 AI 味，就把按钮、横幅、封面占位这些地方的渐变都改成纯色了。

**v7.1 — 间距再对齐**

Element Plus 组件自带的间距在它源码里改不了，只能在外面用优先级更高的样式压过去，把菜单、下拉这些对齐到 8px 网格。仪表盘里几处偏暗的文字也调亮了。

**v7.0 — 重新定了个设计标准**

写了一份 DESIGN-STANDARD.md，规矩不多：间距只用 4/8/12/16/24/32 这几个值，层次靠留白不靠边框，鼠标悬上去只变颜色不变形状。然后照着这个把全站过了一遍，transition:all 这种偷懒写法也全换成了具体属性，弹窗遮罩加了点毛玻璃。

**v6.x — 反复改 UI 的一段**

这段时间被说了好几次"显示正常就行了吗""符合苹果设计理念吗"，前后返工了十来轮。一开始是人工看间距圆角配色，后来上了脚本统计数值分布和对比度，再后来专门弄了弹窗——Element Plus 默认的确认框太素，改成删除用红色、成功提示带个对勾。中间还捎带修了几个真 bug：登录 Token 过期了不跳登录页、管理员调统计接口报 403、仪表盘图表一直是空的（SQL 按最近 30 天查，测试数据都在好几个月前）。

**v5.7 ~ v5.9 — 补漏洞和测试**

修了几个越权问题，读者居然能改别人的密码，说出去太丢人了。功能上加了修改密码、座位时间轴、数据大屏、批量操作这些。前端测试从零写了几十个，接了提交前检查和 SonarQube。

**再往前**

v5.5 整了个"书香"设计系统换配色和字体，v5.4 加了并发锁和 RBAC 权限，v5.2 换 Redis 做缓存、搞了 Docker 部署，v5.0 修过一轮越权。完整的看 CHANGELOG。
