...
## v7.3 (2026-09-14) — 深度溢出审计：文字超出边框清零

### 🔍 新审计工具 overflow_audit.py（针对"肉眼可见但扫样式漏掉"的问题）
- 四类机械检测：文字横向被裁切（无 ellipsis）/ 纵向被裁切 / 子元素超出父容器边框 / 绝对定位元素贴边(<8px)
- 覆盖 3 角色 29 页面 + 12 个弹窗（新增/编辑/删除确认/借阅表单）
- 误报排除：fixed 元素、伪元素装饰裁切、el-input-number 组件固有贴边

### 🛠️ 修复（commit 56bec51）
- 图书管理表格溢出 111px（列宽总和超内容区）→ 11 列压缩至 1090px
- 读者管理表格溢出 6px → ID/操作列压缩
- 弹窗 X 按钮贴死边框（EP 定位相对 dialog 而非 header）→ 距边 12px、36px 见方
- 读者首页推荐理由标签溢出卡片 64px → el-tag 换行收纳
- 读者首页横幅：summary 永不被压（flex-shrink:0），文字侧省略号让位
- 推荐封面裁切 5px → img display:block
- **三角色重跑：真实溢出 0**；vision 复核无任何文字超出边框

---

## v7.2 (2026-09-14) — 全站去渐变色

### 🎨 渐变色清除（去 AI 味）
- 45 处 `linear-gradient` 全部替换为纯色（取渐变主色/首色），12 文件：欢迎横幅、统计卡片图标、座位状态色块、登录页背景、数据大屏面板、侧边栏装饰线
- 无 radial-gradient 残留；build EXIT=0
- vision 验证：纯色界面更清爽利落，层次感通过明暗对比 + 彩色小方块保留，无"太扁平"问题
- Commit `424b3c1`，已 push
